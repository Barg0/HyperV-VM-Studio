...
function Write-DiskInventoryReport {
    ...
    Write-Host ""
    Write-Studio -Text ("  {0,-28} {1,-10} {2,-12} {3,-8} {4,10} {5,10} {6}" -f `
            "VM", "State", "Controller", "Type", "Max", "OnDisk", "File") -Key "accent"
    Write-Studio -Text ("  " + ("-" * 100)) -Key "muted"

    foreach ($d in $Disks) {
        # Palette keys, not ConsoleColor names. Three of these four fell through to
        # `fg`, so Fixed, Dynamic and Differencing were drawn identically in a column
        # whose whole job is to tell them apart. ("Yellow" happened to work, because
        # the palette has a key of that name and the lookup is case insensitive.)
        # The band colours keep the original intent: warm for Fixed, green for
        # Dynamic, violet for Differencing.
        $typeColor = "muted"
        if ($d.IsFixed) { $typeColor = "bandDeploy" }
        elseif ($d.VhdType -eq "Dynamic") { $typeColor = "bandWork" }
        elseif ($d.VhdType -eq "Differencing") { $typeColor = "bandIdent" }

        Write-Host ("  {0,-28} {1,-10} {2,-12} " -f $d.VmName, $d.VmState, $d.Controller) -NoNewline
        Write-Studio -Text ("{0,-8}" -f $d.VhdType) -Key $typeColor -NoNewline
        Write-Host (" {0,10} {1,10} {2}" -f `
                (Format-ByteSize $d.SizeBytes), `
                (Format-ByteSize $d.FileSizeBytes), `
                $d.FileName)
        if ($d.CheckpointCount -gt 0) {
            Write-Studio -Text ("    checkpoints={0} (clear before convert)" -f $d.CheckpointCount) -Key "warn"
        }
        if ($d.VhdError) {
            Write-Studio -Text ("    error: {0}" -f $d.VhdError) -Key "danger"
        }
    }
    Write-Host ""
}
...
function Invoke-ZeroVhdFreeSpace {
    ...
    Write-Log "Zeroing free space in '$Path' - this is slow" -Tag "Run"
...
function Get-GuestCredentialInteractive {
    ...
    Write-Host ""
    Write-Studio -Text "  Guest ReTrim needs PowerShell Direct credentials (local admin inside the VM)." -Key "warn"
    Write-Studio -Text "  Tip: use DOMAIN\user or .\Administrator" -Key "muted"
    Write-Host ""
...
function Convert-FixedDiskToDynamic {
    ...
    try {
        ...
        if (-not $KeepSourceFile) {
            ...
            Write-Log "-KeepSource: kept '$oldPath'" -Tag "Info"
        }
    }
    catch {
        $result.Message = "Post-convert swap failed: $($_.Exception.Message)"
        Write-Log $result.Message -Tag "Error"
        Write-Log "Manual recovery: temp '$tempPath', old '$oldPath'" -Tag "Warn"
...
function Start-ConvertSelectedDisks {
    ...
    foreach ($group in $byVm) {
        ...
        if (-not $WhatIfMode -and $vmReclaimMode -eq "GuestReTrim") {
            ...
            if (-not $retrimOk) {
                Write-Log "ReTrim failed - falling back to ZeroFreeSpace" -Tag "Warn"
...
function Start-CompactSelectedDisks {
    ...
    foreach ($group in $byVm) {
        ...
        if (-not $WhatIfMode -and $vmReclaimMode -eq "GuestReTrim") {
            ...
            $retrimOk = Invoke-GuestVolumeRetrim -VmName $vmName -Credential $cred -Interactive:$Interactive
            if (-not $retrimOk) {
                Write-Log "ReTrim failed - falling back to ZeroFreeSpace" -Tag "Info"
                $vmReclaimMode = "ZeroFreeSpace"
            }
        }
...
function Start-InteractiveConvertMenu {
    while ($true) {
        ...
        Write-Log "Reclaim: ReTrim, then offline ZeroFreeSpace" -Tag "Info"
